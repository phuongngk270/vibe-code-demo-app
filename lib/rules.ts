...
export function collectDeclaredSections(pages: string[]) {
  ...
  pages.forEach((text, i) => {
    text.split('\n').forEach(line => {
      ...
      hits.forEach(tok => {
        const looksHeader =
          /^\s*Section\s+(?:[IVXLCDM]+|\d+|[A-Z])(\s*[-–—:])/.test(line) ||
          line.trimStart().startsWith(tok);
        if (looksHeader) {
...
export function diffMissingReferences(
  declared: Map<string, Set<number>>,
  referenced: Array<{ label: string; page: number; context: string }>
): Issue[] {
  const issues: Issue[] = [];
  for (const r of referenced) {
    if (!declared.has(r.label)) {
      issues.push({
        ...
        original: r.context,
        suggestion: `Remove/update the reference; declared sections: ${
  ...
  pages.forEach((text, idx) => {
    ...
    const flush = () => {
      ...
      if (alpha) {
        for (let i = 1; i < seq.length; i++) {
          ...
          if (curr !== prev + 1)
            issues.push({
              page: idx + 1,
              type: 'numbering',
              message: `List jumps from "${seq[i - 1].lab}" to "${
                seq[i].lab
              }"`,
              original: seq[i].line,
              suggestion: `Insert "${String.fromCharCode(
                prev + 1
              )}." or renumber.`,
        ...
        for (let i = 1; i < seq.length; i++) {
          const p = Number(seq[i - 1].lab);
          const c = Number(seq[i].lab);
          if (Number.isFinite(p) && Number.isFinite(c) && c !== p + 1)
            issues.push({
              page: idx + 1,
              type: 'numbering',
              message: `List jumps from "${p}" to "${c}"`,
              original: seq[i].line,
              suggestion: `Insert "${p + 1}." or renumber.`,
              locationHint: seq[i].line.trim(),
            });
        }
      }
